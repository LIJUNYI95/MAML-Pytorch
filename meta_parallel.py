...
class Meta(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        self.inner_net = nn.ModuleList([Learner(config, args.imgc, args.imgsz) for _ in np.range(self.task_num)])
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        #load net into inner_net
        for i in range(task_num):
            self.inner_net[i].load_state_dict(self.net.state_dict())

        # 1. run the i-th task and compute loss for k=0
        logits = [self.net(x_spt[i], vars=None, bn_training=True) for i in range(self.task_num)]
        loss = [F.cross_entropy(logits[i], y_spt[i]) for i in range(self.task_num)]
        grad = [torch.autograd.grad(loss[i], self.inner_net[i].parameters()) for i in range(self.task_num)]
